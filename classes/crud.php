...
// Inserting
		...
		public function addDiagnosis($value)
	{
		...
		$f2 = isset($post['f2']) ? 1: 0;
		$f3 = isset($post['f3']) ? 1: 0;
		$f4 = isset($post['f4']) ? 1: 0;
		$f5 = isset($post['f5']) ? 1: 0;
		$f6 = isset($post['f6']) ? 1: 0;
		$f7 = isset($post['f7']) ? 1: 0;
		$f8 = isset($post['f8']) ? 1: 0;
		$f9 = isset($post['f9']) ? 1: 0;
		$f10 = isset($post['f10']) ? 1: 0;
		$result = $this->cleanse($post['result']);
		$advice = $this->cleanse($post['advice']);

		$query="INSERT INTO diagnosis(f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,result,advice) VALUES ('$f1','$f2','$f3','$f4','$f5','$f6','$f7','$f8','$f9','$f10','$result','$advice')";
		$sql = $this->con->query($query);
		if ($sql==true) {
		...
		public function compareDiagnosis($post)
	{
		...
		$query = "SELECT * from diagnosis where f1='$f1' and f2='$f2' and f3='$f3' and f4='$f4' and f5='$f5' and f6='$f6' and f7='$f7' and f8='$f8' and f9='$f9' and f10='$f10' ";
		$result = $this->con->query($query);
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			return $row;
